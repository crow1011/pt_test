	...
	for anet in n_allow:
		...
		for dnet in n_deny:
			...
			if dnet.supernet_of(anet):
				...
				# не очень хорошая практика менять объект(список - n_allow) в ходе итерации по его же элементам,
				# в данном случае ничего критичного не происходит, но лучше так не делать совсем,
				# перепиши пожалуйста с этим учетом
